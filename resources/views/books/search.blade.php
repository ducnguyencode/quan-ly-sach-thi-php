...
@section('content')
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center">
            <form action="{{ route('books.search') }}" method="GET" class="relative w-full md:w-64 mr-4">
                <input type="text" name="q" id="searchInput" placeholder="Tìm kiếm sách..." value="{{ $query ?? '' }}"
    ...
    <div class="mb-4">
        <p class="text-gray-600">
            @if(isset($query))
                Kết quả tìm kiếm cho: <span class="font-semibold">{{ $query }}</span>
                @if($books->isEmpty())
                    (Không tìm thấy kết quả nào)
                @else
                    (Tìm thấy {{ $books->total() }} kết quả)
                @endif
            @endif
        </p>
    </div>
    ...
    @if(isset($books) && $books->isEmpty())
        <div class="bg-white rounded-lg shadow-md p-6 text-center">
            ...
            <p class="text-gray-500 mb-4">Không tìm thấy sách nào phù hợp với từ khóa "{{ $query }}".</p>
        </div>
